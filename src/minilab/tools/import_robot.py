#!/usr/bin/env python3
"""Convert a URDF robot to a MiniLab robot MJCF asset directory.

Usage:
  uv run minilab-import-robot <urdf_path> [robot_name]
...
ROBOT_ASSET_ROOT = REPO_ROOT / "src" / "minilab" / "assets" / "robots"
TEMP_MESH_PREFIX = "meshes/meshes/"
DEFAULT_MATERIAL = "default_material"
_FREE_X_HELPER_JOINT = "__minilab_keyframe_x"
_FREE_Y_HELPER_JOINT = "__minilab_keyframe_y"
_HEIGHT_HELPER_JOINT = "__minilab_keyframe_height"
_FREE_BALL_HELPER_JOINT = "__minilab_keyframe_orientation"
_HEIGHT_HELPER_ACTUATOR = "__minilab_keyframe_height_ctrl"
...
def _run(command: Sequence[str], *, check: bool = True) -> subprocess.CompletedProcess[bytes]:
    print(f"[minilab-import-robot] {' '.join(command)}", flush=True)
...
def _ensure_tuning_scene_visuals(root: ET.Element) -> None:
    ...
    if worldbody.find("./light[@name='__minilab_tuning_light']") is None:
        worldbody.append(
            ET.Element(
                "light",
                {
                    "name": "__minilab_tuning_light",
                    "pos": "0 0 1.5",
                    "dir": "0 0 -1",
                    "directional": "true",
                },
...
def _materialize_tuning_scene(
    ...
    *,
    ...
    from minilab.base.backend.mujoco.xml import materialize_scene_fragments
...
def _write_tuned_scene_keyframe(
    ...
    ET.indent(tree, space="  ")
    tree.write(scene_xml, encoding="unicode")
    print(f"[minilab-import-robot] wrote tuned keyframe to {scene_xml.relative_to(REPO_ROOT)}")
...
def convert(urdf_path: str, robot_name: str | None) -> Path:
    ...
    print(f"[minilab-import-robot] wrote {output_xml.relative_to(REPO_ROOT)}", flush=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"[minilab-import-robot] error: {exc}", file=sys.stderr)
        return 1
    return 0
