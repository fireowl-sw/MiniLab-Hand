"""Shell completion helper for MiniLab training entrypoints."""
...
from minilab import cli

TRAINING_ENTRYPOINTS = {
    "train": "minilab.cli:train_main",
    "eval": "minilab.cli:eval_main",
}
...
COMPLETION_BLOCK_START = "# >>> minilab completion >>>"
COMPLETION_BLOCK_END = "# <<< minilab completion <<<"
...
def _demo_completions(
    *,
    ...
    from minilab.demo import DEMO_REGISTRY
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="minilab-complete")
...
def _completion_script_path(shell: str) -> Path:
    ...
    return root / "scripts" / "completions" / f"minilab.{shell}"
...
def _without_completion_block(content: str, *, force: bool = False) -> str:
    ...
    if start == -1 or end == -1 or end < start:
        if force:
            return content
        raise SystemExit(
            "Existing MiniLab completion block is malformed; rerun with --force or edit it manually."
        )
...
def _write_completion_block(
    *,
    ...
    print(f"Installed MiniLab completion in {rc_file}")
...
def _remove_completion_block(*, rc_file: Path, dry_run: bool, force: bool) -> None:
    ...
    if dry_run:
        ...
        return
    rc_file.write_text(updated, encoding="utf-8")
    print(f"Removed MiniLab completion from {rc_file}")


def _install_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="minilab-complete install")
