...
from minilab.assets.hub import resolve_checkpoint_file
...
def build_demo_command(
    *,
    ...
    from minilab.cli import build_command
...
def _mxpython_executable() -> str:
    from minilab.cli import _mxpython_executable as resolve_mxpython_executable
...
def _run_teaser_demo() -> int:
    if platform.system() == "Darwin" and Path(sys.executable).name != "mxpython":
        command = [
            ...
            str(_repo_root() / "src" / "minilab" / "tools" / "render_teaser.py"),
        ]
        env = os.environ.copy()
        env.setdefault("UV_PROJECT_ENVIRONMENT", str(_repo_root() / ".venv"))
        return subprocess.run(command, check=False, env=env).returncode

    from minilab.tools.render_teaser import main as render_teaser_main
