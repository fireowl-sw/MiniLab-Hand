"""Thin package CLI for routing to existing MiniLab training entrypoints."""
...
from minilab.demo import run_demo
...
def _check_private_checkout(root: Path) -> None:
    if not (root / "conf").is_dir() or not (root / "scripts").is_dir():
        raise SystemExit(
            "The current MiniLab CLI expects a MiniLab source checkout. "
