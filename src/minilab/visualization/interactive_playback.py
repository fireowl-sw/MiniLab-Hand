...
def _resolve_appo_checkpoint_from_cfg(
    ...
    *,
    ...
    from minilab.training import get_log_root, resolve_task_checkpoint_path
...
def _build_appo_actor(
    *,
    ...
    if is_hora:
        from minilab.algos.torch.hora.appo import _update_hora_obs_groups
...
def create_appo_playback_session(
    *,
    ...
    from minilab.algos.torch.hora.runtime import is_hora_appo_runtime

    is_hora = is_hora_appo_runtime(rl_cfg)
    selected_wrapper_cls = wrapper_cls
    policy_obs_mode = playback_cfg.policy_obs_mode
    if is_hora:
        from minilab.algos.torch.hora.rsl_rl import HoraRslRlVecEnvWrapper
...
def _default_hora_distill_playback_deps(root_dir: str | Path) -> dict[str, Any]:
    ...
    from minilab.algos.torch.hora.distill import (
        build_student_actor_and_normalizer,
        load_distilled_checkpoint,
    )
    from minilab.algos.torch.hora.rsl_rl import HoraRslRlVecEnvWrapper
    from minilab.training import create_env, get_log_root
